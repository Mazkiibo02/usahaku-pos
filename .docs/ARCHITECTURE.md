...
* **Forced Token Synchronization:** To guarantee that environment migrations or role upgrades apply instantly without serving cached local stale session data, critical write operations (such as processing transaction batches in `pos-service.ts`) programmatically force an auth refresh using `auth.currentUser.getIdToken(true)` right before database execution.
