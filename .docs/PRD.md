...
* **SaaS Paywall Phase 1, 2 & 3 (Midtrans & Fail-Safe Manual Sync):** * Implemented a global reactive UI interceptor (`SubscriptionLock.tsx`) displaying a 3-column pricing tier grid.
...
* **Multi-Outlet Gatekeeping:** * Enforced atomic increment/decrement of `outletsCount` via Firestore `writeBatch` on creation/deletion.
